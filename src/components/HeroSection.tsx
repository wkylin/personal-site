...
export function HeroSection({ profile }: HeroSectionProps) {
  ...
  return (
    <section
      id="hero"
      className="relative isolate w-full overflow-visible rounded-none bg-slate-900/55 px-4 py-7 shadow-[0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:px-6 sm:py-10 lg:px-8"
    >
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            {stack.map((item) => {
              ...
              return (
                <span
                  key={item.label}
                  className="inline-flex min-w-0 items-center gap-1.5 rounded-none border border-white/15 bg-white/6 px-2 py-1.5 text-xs text-slate-200 sm:px-2.5"
                >
